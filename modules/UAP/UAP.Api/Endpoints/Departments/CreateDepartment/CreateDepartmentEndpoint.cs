...
internal class CreateDepartmentEndpoint : EndpointAbstract
{
    ...
    {
        group.MapPost("departments", Execute).WithTags(EndpointTags.Department);
    }

    public async Task<ApiResponse<Guid>> Execute([FromBody] CreateDepartmentRequest request, [FromServices] IMediator mediator, [FromServices] IMapper mapper)
    {
        var command = mapper.Map<CreateDepartmentCommand>(request);

        var departmentId = await mediator.Send(command);
        return ApiResponse.Success(departmentId.Value);
    }
}